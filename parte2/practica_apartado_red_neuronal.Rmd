...
values.nneurons1 <- c(2**4, 2**5, 2**6)
values.nneurons2 <- c(0, 2**4, 2**5, 2**6)
...
mean_accuracy_sd <- foreach(i = 1:nrow(v), .combine = rbind, .packages = c("keras", "MLmetrics")) %dopar% {
  ...
  for (k in seq(1:length(folds))) {
    ...
    modnn %>% fit(x_train, y_train, epochs = v[i, 3], verbose = 0)
...
# hacer ncol(mydf) - 1
# pasar mydf a is.numeric(mydf) (coger solo variables numericas)
# hacer one-standard-error