...
--Falta completar el 10, añadir grupos de act_data_pkt_fwd y min_seg_size_forward, revisar si coinciden el número de variables con las mencionadas aquí y ordenar grupos por orden de aparición en el dataset (opcional). La variable Fwd.Header.Length.1 sobra en el dataset porque es igual a Fwd.Header.Length--
...
- Con el modelo de Bagging se predice correctamente el 86.63% de las conexiones benignas y el 49.42% de las conexiones keylogger. Por otro lado, de los casos predichos como benignos el 71.89% eran correctos y de los casos predichos como keylogger 72.18%.
- Con el modelo de Red Neuronal se predice correctamente el 78.48% de las conexiones benignas y el 46.89 de las conexiones keylogger. Por otro lado, de los casos predichos como benignos el 68.32 eran correctos y de los casos predichos como keylogger 59.87%.

Con este análisis podemos ver que el modelo de Bagging ha obtenido mejores resultados que el modelo de Red Neuronal, tanto en precisión como en sensibilidad. Se puede ver tanto en el F1-Score como en la matriz de confusión. Aún así, ni siquiera el modelo de Bagging ha sido realmente bueno prediciendo las conexiones keylogger, con solo una precisión del 49.42%. Esto es muy cercano a elegir al azar (50% - 50%) si una conexión es maligna o no, que sería una probabilidad del 48% (ya que hay un 48% de casos positivos y un 52% de negativos en el conjunto de entrenamiento).
