...
cl <- makeCluster(4) # Leave one core free
...
mean_accuracy <- foreach(i = 1:nrow(v), .combine = c, .packages = c("randomForest", "MLmetrics")) %dopar% {
  test_fold <- rep(0, length(folds))
  for (k in seq(1:length(folds))) {
    ...
    test_fold[k] <- Accuracy(yhat.bag, folds[[k]]$Class)
  }
...
for (i in 1:dim(v)[1]) {
  for (k in seq(1:length(folds))) {
    ...
    bag <- randomForest(Class ~ .,
      data = data,
      mtry = v[i, 1], ntree = v[i, 2], maxnodes = v[i, 3], nodesize = v[i, 4]
    )
...
results <- data.frame(mean_accuracy = mean_accuracy, mtry = v[, 1], ntree = v[, 2], maxnodes = v[, 3], nodesize = v[, 4])

results

write.csv(results, "results_bagging.csv", row.names = FALSE)

plot(results$mean_accuracy)
```
