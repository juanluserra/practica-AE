...
Vemos que la proporción de casos positivos y negativos en el dataset sampleado es casi idéntica a la del dataset original. Por lo tanto, el dataset sampleado sirve para trabajar. Vamos a guardar el dataset sampleado.
```{r, eval = FALSE}
...
write.csv(train.time.df, "train_time.csv", row.names = FALSE)
...
write.csv(crossvalidation.results, "crossvalitadion_bagging.csv", row.names = FALSE)
...
Una vez tenemos los hiperparámetros seleccionados, vamos a entrenar el modelo con el conjunto de entrenamiento y a realizar las predicciones con el conjunto de test.
```{r}
