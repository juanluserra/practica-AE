...
crossvalidation.results <- foreach(i = 1:nrow(v), .combine = rbind, .packages = c("keras", "MLmetrics")) %dopar% {
  ...
  for (k in seq(1:length(folds))) {
    ...
    modnn %>% fit(x.train, y.train, epochs = v[i, 4], verbose = 0, class_weight = class_weights)
...
py$modnn <- modnn
py_run_string("modnn.save('neural_network_model.keras')")
```

```{r}
# Cargamos la red neuronal de Python y lo pasamos a R
py_run_string("from tensorflow.keras.models import load_model")
py_run_string("model = load_model('neural_network_model.keras')")
model <- py$model
